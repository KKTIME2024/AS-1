...
document.addEventListener('DOMContentLoaded', function() {
    // 获取支出表单元素
    const expenseForm = document.querySelector('form[action="' + window.location.pathname + '/add_expenditure"]') || document.querySelector('form[action="/add_expenditure"]');
    ...
    // 增强表单键盘导航
    if (expenseForm) {
        // Enter键在金额输入框时聚焦到类别选择区域
        amountInput?.addEventListener('keydown', function(e) {
            if (e.key === 'Enter' && !e.shiftKey) {
                e.preventDefault();
                const categorySection = document.querySelector('.category-tags');
                if (categorySection) {
                    const firstRadio = categorySection.querySelector('input[type="radio"]');
